...
def acroform_text_field_stale_appearance_pdf() -> bytes:
    pdf = Pdf()
    content = b""
    appearance = b"0.95 0.82 0.72 rg 0 0 60 20 re f 0 0 0 RG 1 w 0.5 0.5 59 19 re S"
    contents = pdf.add(
        f"<< /Length {len(content)} >>\nstream\n".encode("ascii")
        + content
        + b"\nendstream"
    ...
    field = pdf.add(
        "<< /Type /Annot /Subtype /Widget /FT /Tx /T (Name) /V (Changed) "
        "/Rect [30 30 90 50] "
        f"/AP << /N {appearance_object} 0 R >> >>"
    )
    catalog = pdf.add(f"<< /Type /Catalog /Pages {pages} 0 R /AcroForm << /Fields [{field} 0 R] >> >>")
...
def acroform_checkbox_stale_appearance_state_pdf() -> bytes:
    pdf = Pdf()
    content = b""
    yes_appearance = (
        b"1 1 1 rg 0 0 20 20 re f "
    ...
    field = pdf.add(
        "<< /Type /Annot /Subtype /Widget /FT /Btn /T (Agree) /V /Yes /AS /Off "
        "/Rect [20 30 40 50] "
        f"/AP << /N << /Yes {yes_appearance_object} 0 R /Off {off_appearance_object} 0 R >> >> >>"
    )
    catalog = pdf.add(f"<< /Type /Catalog /Pages {pages} 0 R /AcroForm << /Fields [{field} 0 R] >> >>")
...
def main() -> None:
    ...
    write(
        "acroform-text-field-stale-appearance.pdf",
        acroform_text_field_stale_appearance_pdf(),
    )
    ...
    write(
        "acroform-checkbox-stale-appearance-state.pdf",
        acroform_checkbox_stale_appearance_state_pdf(),
    )
    write(
