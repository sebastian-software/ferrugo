...
cargo run -p ferrugo --no-default-features -- extract-corpus-metadata fixtures/generated \
...
cargo run -p ferrugo --no-default-features -- operator-coverage fixtures/generated \
  --manifest fixtures/corpus-manifest.tsv \
  "${family_args[@]}" \
...
cargo run -p ferrugo --no-default-features -- benchmark-batch-native fixtures/generated \
