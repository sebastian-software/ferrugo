...
#[no_mangle]
pub extern "C" fn pdfrust_wasm_smoke_fixture_count() -> u32 {
    SMOKE_FIXTURES.len() as u32
}

/// Renders the smoke suite and returns total RGBA bytes, or `0` on failure.
#[no_mangle]
pub extern "C" fn pdfrust_wasm_smoke_total_output_bytes() -> u32 {
    render_low_memory_smoke()
        .ok()
...
mod tests {
    ...
    fn wasm_smoke_should_render_low_memory_thumbnail_on_host() {
        ...
        assert_eq!(
            pdfrust_wasm_smoke_fixture_count(),
            SMOKE_FIXTURES.len() as u32
        );
        assert_eq!(
            pdfrust_wasm_smoke_total_output_bytes() as usize,
