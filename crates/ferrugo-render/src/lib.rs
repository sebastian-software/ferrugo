...
use ferrugo_syntax::{ByteOffset, PdfBytes, PdfName, PdfNumber, PdfPrimitive, PdfString};
use ferrugo_thumbnail::{PixelFormat, Rgba};
use zune_jpeg::{
...
/// Inclusive-exclusive pixel bounds used to limit path rasterization work.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct RasterScissor {
    /// First writable pixel column.
    pub min_x: u32,
    /// First writable pixel row.
    pub min_y: u32,
    /// Pixel column one past the writable region.
    pub max_x: u32,
    /// Pixel row one past the writable region.
    pub max_y: u32,
}

impl RasterScissor {
    /// Creates inclusive-exclusive raster scissor bounds.
    #[must_use]
...
pub struct PathRasterOptions {
    ...
    pub supersample: u8,
    /// Fill rasterization route for non-pattern path fills.
    pub fill_route: FillRasterRoute,
    /// Optional pixel-space bounds for banded or scissored path rasterization.
    pub scissor: Option<RasterScissor>,
...
/// Rasterization route for ordinary filled paths.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FillRasterRoute {
    /// Use the existing per-pixel supersampled point-in-path route.
    Sampled,
    /// Use coverage spans for full interior pixels and sample only edge pixels.
    CoverageSpans,
}
...
pub struct StrokeShapeSummary {
    ...
    pub simple_line_span_below_threshold_pixels: usize,
    /// Number of stroked items expected to fall through to the generic stroke loop.
    pub generic_stroke_fallback_items: usize,
...
impl StrokeShapeSummary {
    ...
    pub fn merge(&mut self, other: Self) {
        ...
        self.generic_stroke_fallback_items += other.generic_stroke_fallback_items;
...
/// Runtime fill-raster route summary for renderer performance diagnostics.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct FillRasterRouteSummary {
    /// Number of non-pattern fill calls handled by the sampled route.
    pub sampled_calls: usize,
    /// Number of sampled calls caused by complex clips.
    pub complex_clip_fallback_calls: usize,
    /// Number of non-pattern fill calls handled by coverage spans.
    pub coverage_span_calls: usize,
    /// Raster rows visited by coverage-span fill calls.
    pub coverage_span_rows: usize,
    /// Filled intervals generated by scanline coverage.
    pub coverage_span_intervals: usize,
    /// Contiguous full-coverage row runs emitted by the coverage route.
    pub coverage_full_span_runs: usize,
    /// Pixels written as fully covered interior spans.
...
impl FillRasterRouteSummary {
    ...
    fn record_coverage_span_call(&mut self, stats: FillCoverageSpanStats) {
        self.coverage_span_calls += 1;
        self.coverage_span_rows = self.coverage_span_rows.saturating_add(stats.rows);
        self.coverage_span_intervals = self.coverage_span_intervals.saturating_add(stats.intervals);
        self.coverage_full_span_runs = self
            .coverage_full_span_runs
            .saturating_add(stats.full_span_runs);
        self.coverage_full_pixels = self.coverage_full_pixels.saturating_add(stats.full_pixels);
...
pub struct StrokeRasterRouteSummary {
    /// Number of stroke calls converted to fill outlines.
...
impl StrokeRasterRouteSummary {
    fn record_outline_fill_call(&mut self, axis_line: bool) {
...
fn stroke_shape_summary_for_path(
    ...
    let mut summary = StrokeShapeSummary {
        ...
        axis_span_routed_items: usize::from(axis_span_routed),
        simple_line_span_routed_items: usize::from(simple_line_span_routed),
        simple_line_span_below_threshold_items: usize::from(simple_line_span_below_threshold),
        simple_line_span_below_threshold_pixels: if simple_line_span_below_threshold {
            simple_line_pixel_area
        } else {
            0
        },
        generic_stroke_fallback_items: usize::from(generic_stroke_fallback),
...
/// Rasterizes all supported display-list items and records fill/stroke route counts.
///
/// This is intended for explicit diagnostics. Normal rendering should use
/// [`rasterize_display_list_into`] or
/// [`rasterize_display_list_into_with_phase_timings`].
...
///
/// Returns [`RasterError`] when path, image, or text rasterization fails.
pub fn rasterize_display_list_into_with_phase_timings_and_route_summaries(
    display_list: &DisplayList,
    device: &mut RasterDevice,
    transform: PageTransform,
    options: PathRasterOptions,
...
#[expect(
    clippy::too_many_arguments,
    reason = "keeps ordered raster scratch, tracing sinks, and timing callback explicit"
)]
...
struct PathRasterContext<'a> {
    ...
    options: PathRasterOptions,
    clips: &'a [ActiveClip],
    fill_routes: Option<&'a RefCell<FillRasterRouteSummary>>,
    stroke_routes: Option<&'a RefCell<StrokeRasterRouteSummary>>,
}

impl PathRasterContext<'_> {
    fn record_fill_sampled_call(self) {
        if let Some(fill_routes) = self.fill_routes {
            fill_routes.borrow_mut().record_sampled_call();
        }
    }
...
fn fill_path(
    ...
    if context.options.fill_route == FillRasterRoute::CoverageSpans
        && fill_path_with_coverage_spans(device, path, rule, source, blend_mode, alpha, context)?
    {
        return Ok(());
    }
    context.record_fill_sampled_call();
    fill_path_sampled(device, path, rule, source, blend_mode, alpha, context)
}

fn fill_path_sampled(
    device: &mut RasterDevice,
    path: &FlattenedPath,
    rule: FillRule,
    source: Rgba,
...
fn fill_path_with_coverage_spans(
    device: &mut RasterDevice,
    path: &FlattenedPath,
    rule: FillRule,
    source: Rgba,
    ...
    let mut clip_mask = FillCoverageClipMask::new(context.clips, bounds);
    ...
    for y in bounds.min_y..bounds.max_y {
        stats.rows += 1;
        row_modes.fill(FILL_ROW_EMPTY);
        append_fill_intervals_for_row(
            path,
            rule,
            f64::from(y) + 0.5,
            &mut intersections,
        ...
        while offset < row_modes.len() {
            let mode = row_modes[offset];
            let x = bounds.min_x + offset as u32;
            match mode {
                FILL_ROW_FULL => {
                    let run_end = row_modes[offset..]
                        .iter()
                    ...
                    if let Some(clip_mask) = clip_mask.as_mut() {
                        blitter.write_masked_full_span(
                            device,
                            y,
                            x,
                            max_x,
                            clip_mask.row_slice(y, x, max_x),
                            &mut stats,
                        )?;
                    } else {
    ...
    context.record_fill_coverage_span_call(stats);
    Ok(true)
}

const FILL_ROW_EMPTY: u8 = 0;
const FILL_ROW_SAMPLED: u8 = 1;
const FILL_ROW_FULL: u8 = 2;
...
fn append_fill_intervals_for_row(
    path: &FlattenedPath,
    rule: FillRule,
    scan_y: f64,
    intersections: &mut Vec<f64>,
    intervals: &mut Vec<FillInterval>,
) {
    intersections.clear();
    intervals.clear();
    for subpath in &path.subpaths {
        for edge in polygon_edges(subpath) {
            if edge_crosses_scanline(edge, scan_y) {
                let ratio = (scan_y - edge.from.y) / (edge.to.y - edge.from.y);
                intersections.push((edge.to.x - edge.from.x).mul_add(ratio, edge.from.x));
            }
        }
    }
    ...
    intersections.dedup_by(|left, right| (*left - *right).abs() <= 1e-9);
    for pair in intersections.windows(2) {
        ...
        if point_in_path(midpoint, path, rule) {
            intervals.push(FillInterval { min_x, max_x });
        }
    }
}

fn edge_crosses_scanline(edge: LineSegment, scan_y: f64) -> bool {
...
#[derive(Debug, Clone)]
struct FillCoverageClipMaskRow {
    coverage: Vec<u8>,
    computed: Vec<bool>,
}

impl FillCoverageClipMaskRow {
    fn new(width: usize) -> Self {
        Self {
            coverage: vec![0; width],
            computed: vec![false; width],
        }
    }
}

impl<'a> FillCoverageClipMask<'a> {
    fn new(clips: &'a [ActiveClip], bounds: PixelBounds) -> Option<Self> {
    ...
    fn coverage(&mut self, x: u32, y: u32) -> u8 {
    ...
    fn row_slice(&mut self, y: u32, min_x: u32, max_x: u32) -> &[u8] {
        debug_assert!(y >= self.bounds.min_y && y < self.bounds.max_y);
        debug_assert!(min_x >= self.bounds.min_x);
        debug_assert!(max_x <= self.bounds.max_x);
...
fn sampled_fill_coverage_for_pixel(
    ...
    for sample_y in 0..samples {
        for sample_x in 0..samples {
            ...
            if point_in_active_clips(point, context.clips) && point_in_path(point, path, rule) {
                covered += 1;
            }
        }
    }
...
fn fill_axis_aligned_rect_path(
    ...
    if let Some(mut bounds) = center_sampled_rect_pixel_bounds(rect, dimensions) {
        let Some(scissored_bounds) =
            intersect_scissor_pixel_bounds(bounds, context.options.scissor, dimensions)
        else {
            return Ok(());
        };
        bounds = scissored_bounds;
    ...
    let Some(bounds) = device_pixel_bounds(rect, dimensions, 0.0).and_then(|bounds| {
        intersect_active_clip_pixel_bounds(
            bounds,
            context.clips,
            dimensions,
...
impl CoverageDrawBlitter {
    ...
    fn write_masked_full_span(
        ...
        debug_assert_eq!(mask.len(), (max_x - min_x) as usize);
        let pixels = mask.len();
        stats.full_span_runs += 1;
        stats.full_pixels = stats.full_pixels.saturating_add(pixels);
        ...
        while offset < mask.len() {
            match mask[offset] {
                ...
                255 => {
                    let run_len = mask[offset..]
                        .iter()
                        .position(|coverage| *coverage != 255)
                        .unwrap_or(mask.len() - offset);
                    self.write_untracked_full_span(
                        device,
                        y,
                        min_x + offset as u32,
                        min_x + (offset + run_len) as u32,
                    )?;
                    stats.clip_mask_pixels = stats.clip_mask_pixels.saturating_add(run_len);
                    self.record_full_span_kind_pixels(stats, run_len);
                    offset += run_len;
                }
                _ => {
                    let run_len = mask[offset..]
    ...
    fn record_full_span_kind_pixels(self, stats: &mut FillCoverageSpanStats, pixels: usize) {
        match self.kind {
            CoverageDrawBlitterKind::OpaqueNormal => {
                stats.direct_row_pixels = stats.direct_row_pixels.saturating_add(pixels);
            }
    ...
    fn write_coverage_alpha_chunk(self, chunk: &mut [u8], coverage_alpha: u8) {
        if coverage_alpha == 0 {
            return;
        }
        let coverage = (self.alpha * f64::from(coverage_alpha) / 255.0).clamp(0.0, 1.0);
        if coverage <= f64::EPSILON {
            return;
        }
        if coverage >= 1.0 && matches!(self.kind, CoverageDrawBlitterKind::OpaqueNormal) {
            chunk.copy_from_slice(&[self.source.r, self.source.g, self.source.b, self.source.a]);
            return;
        }
        let dest = Rgba {
            r: chunk[0],
        ...
        let blended = match self.kind {
            CoverageDrawBlitterKind::OpaqueNormal | CoverageDrawBlitterKind::SourceOverNormal => {
                if dest.a == 255 {
                    source_over_opaque(self.source, dest, coverage)
                } else {
                    source_over(self.source, dest, coverage)
                }
            }
            CoverageDrawBlitterKind::NormalAlpha => source_over(self.source, dest, coverage),
            CoverageDrawBlitterKind::Multiply => source_over(
                Rgba {
                    a: self.source.a,
                    ..multiply_blend_pixel(self.source, dest)
                },
                dest,
                coverage,
            ),
            CoverageDrawBlitterKind::Screen => source_over(
                Rgba {
                    a: self.source.a,
                    ..screen_blend_pixel(self.source, dest)
                },
                dest,
                coverage,
            ),
...
fn fill_pixel_bounds_opaque(
    ...
    for y in bounds.min_y..bounds.max_y {
        fill_opaque_row_span(device, y, bounds.min_x, bounds.max_x, source)?;
    }
    Ok(())
}

fn fill_opaque_row_span(
    device: &mut RasterDevice,
    y: u32,
    min_x: u32,
    max_x: u32,
...
fn fill_axis_aligned_rect_with_tiling_pattern(
    ...
    let Some(scissored_bounds) =
        intersect_scissor_pixel_bounds(bounds, context.options.scissor, device.dimensions())
    else {
        return Ok(true);
    };
    bounds = scissored_bounds;
...
fn intersect_active_clip_pixel_bounds(
    ...
    clips: &[ActiveClip],
    dimensions: RasterDimensions,
    scissor: Option<RasterScissor>,
) -> Option<PixelBounds> {
    bounds = intersect_scissor_pixel_bounds(bounds, scissor, dimensions)?;
    for clip in clips {
...
fn intersect_scissor_pixel_bounds(
    bounds: PixelBounds,
    scissor: Option<RasterScissor>,
    dimensions: RasterDimensions,
) -> Option<PixelBounds> {
    ...
    intersect_pixel_bounds(bounds, scissor_pixel_bounds(scissor, dimensions)?)
}

fn scissor_pixel_bounds(
    scissor: RasterScissor,
    dimensions: RasterDimensions,
) -> Option<PixelBounds> {
    let min_x = scissor.min_x.min(dimensions.width);
    let min_y = scissor.min_y.min(dimensions.height);
    let max_x = scissor.max_x.min(dimensions.width);
    let max_y = scissor.max_y.min(dimensions.height);
    (min_x < max_x && min_y < max_y).then_some(PixelBounds {
        min_x,
        min_y,
        max_x,
...
fn rasterize_simple_line_stroke_spans(
    ...
    for y in bounds.min_y..bounds.max_y {
        x_ranges.clear();
        for sample_y in 0..samples {
            append_axis_stroke_pixel_x_ranges(&spans, bounds, y, sample_y, &mut x_ranges);
        }
        merge_pixel_ranges(&mut x_ranges);
        for x_range in &x_ranges {
            for x in x_range.clone() {
...
fn simple_line_stroke_raster_spans(
    ...
    let mut spans = Vec::with_capacity(end.saturating_sub(start).min(row_count));
    for sample_row in 0..row_count {
        let row_start = spans.len();
        if (start..end).contains(&sample_row) {
            let sample_y =
                f64::from(stroke_bounds.min_y) + (sample_row as f64 + 0.5) / f64::from(samples);
            if let Some(span) =
                simple_line_stroke_span_for_sample_y(line, radius, line_cap, sample_y)
            {
                spans.push(span);
            }
        }
        rows.push(row_start..spans.len());
    }
    (!spans.is_empty()).then_some(AxisStrokeSpans {
...
fn simple_line_stroke_span_for_sample_y(
    ...
    if is_axis_aligned_line(line) {
        return axis_stroke_span_for_sample_y(line, radius, line_cap, sample_y);
    }

    let center_line = match line_cap {
        LineCap::Butt | LineCap::Round => line,
        LineCap::Square => square_capped_line_segment(line, radius),
    };
    let mut span = stroke_quad_span_for_sample_y(center_line, radius, sample_y);
    if matches!(line_cap, LineCap::Round) {
        span = merge_optional_stroke_span(
            span,
            round_cap_span_for_sample_y(line.from, radius, sample_y),
        );
        span = merge_optional_stroke_span(
            span,
            round_cap_span_for_sample_y(line.to, radius, sample_y),
        );
    }
    span
}
...
fn stroke_quad_span_for_sample_y(
    ...
    let normal_x = -dy / length * radius;
    let normal_y = dx / length * radius;
    let corners = [
        Point {
            x: line.from.x + normal_x,
            y: line.from.y + normal_y,
        },
        Point {
            x: line.to.x + normal_x,
            y: line.to.y + normal_y,
        },
        Point {
            x: line.to.x - normal_x,
            y: line.to.y - normal_y,
    ...
    for index in 0..corners.len() {
        ...
        if (from.y - to.y).abs() <= f64::EPSILON {
            if (sample_y - from.y).abs() <= f64::EPSILON {
...
fn round_cap_span_for_sample_y(
    ...
    let delta_y = (sample_y - center.y).abs();
    if delta_y > radius {
        return None;
    }
    let half_width = (radius * radius - delta_y * delta_y).max(0.0).sqrt();
    Some(AxisStrokeSpan {
...
fn merge_optional_stroke_span(
    ...
    match (left, right) {
        (Some(left), Some(right)) => Some(AxisStrokeSpan {
...
impl OpaqueRgbSampleCache {
    fn sample(&mut self, image: &ImageXObject, sample_x: u32, sample_y: u32) -> Rgba {
        ...
        let color = sample_opaque_device_rgb_at(image, sample_x, sample_y);
        self.last = Some(CachedImageSample {
            x: sample_x,
            y: sample_y,
            color,
...
fn sample_opaque_device_rgb_at(image: &ImageXObject, sample_x: u32, sample_y: u32) -> Rgba {
    let index = (sample_y as usize * image.width as usize + sample_x as usize) * 3;
    Rgba {
...
impl GraphicsError {
    ...
    fn from_content(error: ferrugo_content::ContentError) -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn simple_line_stroke_raster_spans_should_cover_single_line_strokes() {
        let dimensions = RasterDimensions::new(48, 48).expect("valid dimensions");
        let line = LineSegment {
            from: Point { x: 6.25, y: 9.5 },
            to: Point { x: 39.75, y: 31.25 },
        };
        let radius = 0.75;
        ...
        for line_cap in [LineCap::Butt, LineCap::Round, LineCap::Square] {
            let spans =
                simple_line_stroke_raster_spans(line, radius, dimensions, bounds, 2, line_cap)
                    .expect("simple line spans");
            let mut candidate_samples = 0;
                ...
                for x in bounds.min_x..bounds.max_x {
                    for sample_y in 0..2 {
                        for sample_x in 0..2 {
                            total_samples += 1;
                            let point = sample_point(x, y, sample_x, sample_y, 2);
                            let candidate = point_in_axis_stroke_spans(point, y, sample_y, &spans);
                            if candidate {
                                candidate_samples += 1;
                            }
                            assert!(
                                candidate
                                    || !point_in_stroke(point, &[line], radius, line_cap),
                                "span missed {line_cap:?} stroke sample at pixel ({x},{y}) sample ({sample_x},{sample_y})"
                            );
                        }
                    }
                }
    ...
    #[test]
    fn stroke_raster_route_summary_should_count_outline_fill_calls() {
        let transform = PageTransform::new(
            PageGeometry {
                media_box: PathBounds {
                    min_x: 0.0,
        ...
        let display_list = DisplayList::from_items(vec![DisplayItem::Path(PathDisplayItem {
            segments: vec![
                ...
                PathSegment::LineTo(Point { x: 100.0, y: 20.0 }),
            ],
            paint: PaintMode::Stroke,
            state: GraphicsState {
                line_width: 4.0,
    ...
    fn intersect_active_clip_pixel_bounds_should_limit_raster_bounds() {
        ...
        assert_eq!(
            intersect_active_clip_pixel_bounds(paint_bounds, &clips, dimensions, None),
            Some(PixelBounds {
                min_x: 4,
                min_y: 2,
                max_x: 10,
    ...
    fn graphics_state_should_ignore_text_fixture_operators() {
        let state = interpret_graphics_state(
            tokenize_content(PdfBytes::new(
                b"BT /F1 24 Tf 40 90 Td (ferrugo thumbnail fixture) Tj ET",
            )),
            GraphicsStateOptions::default(),
        )
        ...
        FlattenedPath {
            ...
            lines: vec![
                LineSegment {
                    ...
                    to: Point { x: 13.0, y: 1.0 },
                },
                LineSegment {
                    from: Point { x: 13.0, y: 1.0 },
                    to: Point { x: 10.0, y: 10.0 },
                },
                LineSegment {
                    from: Point { x: 10.0, y: 10.0 },
                    to: Point { x: 4.0, y: 10.0 },
                },
                LineSegment {
                    from: Point { x: 4.0, y: 10.0 },
                    to: Point { x: 2.0, y: 1.0 },
                },
            ],
            stats: FlattenedPathStats::default(),
            joins: Vec::new(),
    ...
    fn fill_path_coverage_spans_should_count_analytic_edges_and_row_work() {
        ...
        assert!(fill_routes.coverage_span_intervals > 0);
        assert!(fill_routes.coverage_full_span_runs > 0);
        assert!(fill_routes.coverage_full_pixels > 0);
        assert_eq!(
    ...
    #[test]
    fn fill_path_coverage_spans_should_row_blit_full_clip_mask_runs() {
        let path = coverage_span_test_path();
        let transform = coverage_span_test_transform();
        let clip_path = FlattenedPath {
            subpaths: vec![vec![
            ...
            lines: vec![
                LineSegment {
                    from: Point { x: 0.0, y: 0.0 },
                    to: Point { x: 16.0, y: 0.0 },
                },
                LineSegment {
                    from: Point { x: 16.0, y: 0.0 },
                    to: Point { x: 16.0, y: 12.0 },
                },
                LineSegment {
                    from: Point { x: 16.0, y: 12.0 },
                    to: Point { x: 0.0, y: 12.0 },
                },
                LineSegment {
                    from: Point { x: 0.0, y: 12.0 },
                    to: Point { x: 0.0, y: 0.0 },
                },
            ],
            stats: FlattenedPathStats::default(),
    ...
    #[test]
    fn fill_path_should_respect_raster_scissor_bounds() {
        let path = FlattenedPath {
            subpaths: vec![vec![
                Point { x: 1.0, y: 1.0 },
                Point { x: 11.0, y: 1.0 },
            ...
            lines: vec![
                LineSegment {
                    ...
                    to: Point { x: 11.0, y: 1.0 },
                },
                LineSegment {
                    from: Point { x: 11.0, y: 1.0 },
                    to: Point { x: 11.0, y: 11.0 },
                },
                LineSegment {
                    from: Point { x: 11.0, y: 11.0 },
                    to: Point { x: 1.0, y: 11.0 },
                },
                LineSegment {
                    from: Point { x: 1.0, y: 11.0 },
        ...
        .expect("valid transform");
        let mut device = RasterDevice::new(12, 12, Rgba::WHITE).expect("valid raster");
        ...
        fill_path(
            ...
            PathRasterContext {
                transform,
                options: PathRasterOptions {
                    scissor: Some(RasterScissor::new(4, 3, 8, 9)),
                    ..PathRasterOptions::default()
                },
                clips: &[],
                fill_routes: None,
        ...
        .expect("scissored fill should rasterize");
        let black = Rgba {
            r: 0,
            g: 0,
            b: 0,
        ...
        assert_eq!(device.pixel(5, 5).expect("inside scissor"), black);
        assert_eq!(device.pixel(3, 5).expect("left of scissor"), Rgba::WHITE);
        assert_eq!(device.pixel(8, 5).expect("right of scissor"), Rgba::WHITE);
        assert_eq!(device.pixel(5, 2).expect("above scissor"), Rgba::WHITE);
        assert_eq!(device.pixel(5, 9).expect("below scissor"), Rgba::WHITE);
    }
    ...
    fn text_display_list_should_parse_generated_text_fixture() {
        ...
        assert_eq!(text.text, "ferrugo thumbnail fixture");
    ...
    fn image_resources_should_enforce_total_image_byte_budget() {
        ...
        let xobjects = vec![
            ...
            (
                PdfName::new(b"Im2"),
                PdfPrimitive::Reference(ferrugo_syntax::PdfReference::new(5, 0)),
            ),
        ];
    ...
    #[test]
    fn image_fast_path_should_only_accept_opaque_device_rgb_samples() {
        ...
        let xobjects = resources
            ...
            .map(|(name, object)| {
                (
                    ...
                    PdfPrimitive::Reference(ferrugo_syntax::PdfReference::new(*object, 0)),
                )
            })
            .collect::<Vec<_>>();
    ...
    fn minimal_type1_font(charstring: &[u8]) -> Vec<u8> {
        let mut font = b"%!PS-AdobeFont-1.0: FerrugoType1 1.0\n/CharStrings 2 dict dup begin\n/.notdef <0e> def\n/A <".to_vec();
