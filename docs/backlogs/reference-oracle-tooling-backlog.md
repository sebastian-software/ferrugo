...
| Golden artifact retention policy | Done in `docs/policies/native-golden-images.md`. | Policy caps dimensions, bytes, fixture eligibility, and update process; larger artifacts stay local. |
...
| CI sample golden set | Done in `fixtures/native-golden-manifest.tsv`. | A manifest-driven gate covers 32 generated samples across all generated corpus families plus one public real-world scan sample with native-only CI. |
