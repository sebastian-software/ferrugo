...
```sh
env -u FERRUGO_PDFIUM_LIBRARY -u DYLD_LIBRARY_PATH cargo run -p ferrugo-cli --no-default-features -- render fixtures/generated/vector-paths.pdf --max-edge 120 --output target/pdfium-delete-0141-render.png
```
...
```sh
env -u FERRUGO_PDFIUM_LIBRARY -u DYLD_LIBRARY_PATH cargo run -p ferrugo-cli --no-default-features -- render fixtures/generated/optional-content-ocmd.pdf --allow-pdfium-fallback --max-edge 120 --output target/pdfium-delete-0141-rejected.png
```
...
```sh
FERRUGO_PDFIUM_LIBRARY=/private/tmp/ferrugo-tools/pdfium-work/pdfium/out/ferrugo-dylib/libpdfium.dylib DYLD_LIBRARY_PATH=/private/tmp/ferrugo-tools/pdfium-work/pdfium/out/ferrugo-dylib cargo run -p ferrugo-cli --features pdfium -- render-pdfium fixtures/generated/vector-paths.pdf --max-edge 120 --output target/pdfium-delete-0141-pdfium-render.png
```
...
```sh
cargo fmt --check
cargo test -p ferrugo-cli render_auto -- --nocapture
cargo test -p ferrugo-cli render_config_should_reject_explicit_pdfium_fallback_flag -- --nocapture
