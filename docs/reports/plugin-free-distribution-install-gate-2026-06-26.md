...
| Check | Result |
| --- | --- |
| Native-only dependency graph | No `ferrugo-pdfium` edge for `ferrugo-cli --no-default-features`. |
...
| Clean install smoke | `cargo install --path crates/ferrugo-cli --no-default-features --locked` renders `fixtures/generated/text-page.pdf` without PDFium env vars. |
...
cargo install --path crates/ferrugo-cli --no-default-features --locked --root target/install-0157 --force
env -u FERRUGO_PDFIUM_LIBRARY -u DYLD_LIBRARY_PATH target/install-0157/bin/ferrugo-cli render fixtures/generated/text-page.pdf --max-edge 96 --output target/install-0157/text-page.png
cargo fmt --check
