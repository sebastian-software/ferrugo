...
new `pdfrust-wasm-smoke` crate is publish-disabled, depends only on
`pdfrust-native` and `pdfrust-thumbnail`, and builds a `cdylib` for
`wasm32-unknown-unknown`. It intentionally excludes `pdfrust-cli` and
`pdfrust-pdfium`, so PDFium dynamic-library fallback is not part of the WASM
package path.

The smoke crate renders `text-page.pdf` through `NativeBackend::low_memory()`
at `max_edge 96`, exposes `pdfrust_wasm_smoke_status` for a browser or JS
...
- `pdfrust-pdfium` is not a dependency of the smoke crate and is not included
  in the default workspace members.
- `pdfrust-cli` remains a native command-line surface and is not the WASM
  package entrypoint.
...
```text
cargo check -p pdfrust-wasm-smoke --target wasm32-unknown-unknown --no-default-features
cargo build -p pdfrust-wasm-smoke --target wasm32-unknown-unknown --release
node scripts/wasm_smoke.mjs target/wasm32-unknown-unknown/release/pdfrust_wasm_smoke.wasm target/wasm-0132-smoke.json
```

The default budgets are configurable with:

```text
PDFRUST_WASM_MAX_BYTES
PDFRUST_WASM_MAX_COMPILE_MS
PDFRUST_WASM_MAX_INSTANTIATE_MS
PDFRUST_WASM_MAX_SMOKE_MS
```
...
cargo test -p pdfrust-wasm-smoke -- --nocapture
sh scripts/check_wasm_smoke.sh
cargo test --workspace
cargo test --workspace --no-default-features
git diff --check -- Cargo.toml Cargo.lock crates/pdfrust-wasm-smoke/Cargo.toml crates/pdfrust-wasm-smoke/src/lib.rs scripts/check_wasm_smoke.sh scripts/wasm_smoke.mjs
```
