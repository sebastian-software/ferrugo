...
```text
cargo fmt --check
git diff --check -- crates/ferrugo-native/src/lib.rs fixtures/corpus-manifest.tsv fixtures/longform-text-manifest.tsv scripts/generate_fixtures.py
cargo check --workspace
cargo test -p ferrugo-native longform -- --nocapture
...
FERRUGO_PDFIUM_LIBRARY=/private/tmp/ferrugo-tools/pdfium-work/pdfium/out/ferrugo-dylib/libpdfium.dylib DYLD_LIBRARY_PATH=/private/tmp/ferrugo-tools/pdfium-work/pdfium/out/ferrugo-dylib cargo run -p ferrugo-cli --features pdfium -- visual-diff fixtures/generated --manifest fixtures/longform-text-manifest.tsv --include-family book --include-family manual --include-family ebook --include-family repeated-resources --max-edge 160 --max-mae 2.0 --max-p95 16 --max-changed-ratio 0.05 --output target/longform-0127-visual-diff.json
```
