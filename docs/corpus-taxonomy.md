...
Status: accepted for Phase 9 sampling, updated for #97 real-world tier.
Date: 2026-07-04.

The committed corpus is grouped by document family so native renderer progress
can be reported against realistic document shapes instead of only isolated PDF
features. Generated fixtures stay under the original family names. Committed
real-world fixtures use `real-*` families so synthetic and real public inputs
remain separable in JSON, Markdown, and issue evidence.
...
| `real-office-export` | Public redistributable office/form-style PDFs produced outside the fixture generator. | XFA, JavaScript, tagging, multi-page form layout |
| `real-browser-print` | Public web pages printed through a real browser engine. | Chrome/Skia output, tagged browser PDFs, CSS-to-PDF layout |
| `real-scan` | Public redistributable scanned or image-dominant PDFs. | large pages, image memory pressure, scan geometry |
| `real-report` | Public redistributable report/publication PDFs from production layout tools. | InDesign/PDF Library output, charts, tagging, multi-page reports |
...
Rows under `fixtures/generated/` are generated by
`scripts/generate_fixtures.py` and licensed as `MIT OR Apache-2.0`, matching
this repository. Rows under `fixtures/real-world/` are small public,
redistributable PDFs with source URL, license basis, page count, SHA-256, and
producer notes recorded in `fixtures/real-world/README.md`.

## Real-World Benchmark Tier

`fixtures/real-world/` is the committed #97 seed tier for public
redistributable PDFs. It intentionally starts small:

- one federal form/office-style export;
- one Chrome/Skia browser print;
- one scanned/large-page USPTO patent PDF;
- one public data-report PDF.

The same rows are included in `fixtures/performance-matrix-manifest.tsv`, so a
real-world matrix run uses the same parser and report schema as the generated
matrix:
...
Use `fixtures/local-corpus-matrix.example.tsv` as the template for private
local PDFs that should feed the same matrix schema without being committed.
